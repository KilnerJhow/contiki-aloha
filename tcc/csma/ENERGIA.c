...
PROCESS_THREAD(example_collect_process, ev, data) {
  ...
  char packet[_Nb];
  char seqno_str[20];
  // powertrace_start(periodic_interval);
  ...
  // printf("%lu,%lu,%lu,%lu, seqno: %s\n", cpu, lpm, transmit, listen,
  // seqno_str); sprintf(packet, "%lu,%lu,%lu,%lu,%s", cpu, lpm, transmit,
  // listen, seqno_str);
  sprintf(packet, "%lu,%lu,%lu,%lu", cpu, lpm, transmit, listen);
